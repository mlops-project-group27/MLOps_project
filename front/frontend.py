...
def main():
    ...
    with col1:
        ...
        input_method = st.radio(
            "Choose input method:",
            ["Manual Entry", "Sample Transaction", "Upload CSV"]
        )
        ...
        if input_method == "Manual Entry":
            ...
            feature_df = pd.DataFrame({
                'Feature': [f'V{i+1}' for i in range(28)],
                'Value': features
            })
            ...
            for i, col in enumerate(cols):
                ...
                with col:
                    st.dataframe(
                        feature_df.iloc[start_idx:end_idx],
                        hide_index=True,
    ...
    with col2:
        ...
        if 'prediction_result' in st.session_state:
            ...
            if is_fraud:
                ...
                st.markdown(
                    f'<div style="background-color: #ffebee; padding: 15px; border-radius: 5px; border-left: 4px solid #f44336;">'
                    f'<h4 style="color: #d32f2f; margin: 0;">High Risk Transaction</h4>'
                    f'<p style="margin: 5px 0 0 0;">This transaction shows suspicious patterns</p>'
                    f'</div>',
                ...
                st.markdown(
                    f'<div style="background-color: #e8f5e8; padding: 15px; border-radius: 5px; border-left: 4px solid #4caf50;">'
                    f'<h4 style="color: #388e3c; margin: 0;">Normal Transaction</h4>'
                    f'<p style="margin: 5px 0 0 0;">This transaction appears legitimate</p>'
                    f'</div>',
                    unsafe_allow_html=True
                )
            ...
            error_percentage = min(100, (reconstruction_error / (threshold * 2)) * 100)
            ...
            st.markdown(
                f'<div style="text-align: center; padding: 10px; background-color: {risk_color}20; border-radius: 5px; margin-top: 10px;">'
                f'<h3 style="color: {risk_color}; margin: 0;">{risk_level}</h3>'
                f'</div>',
                unsafe_allow_html=True
            )
        else:
